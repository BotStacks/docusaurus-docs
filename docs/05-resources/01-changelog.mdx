...
~ **New:**
Introduced multi-mode Assistant deployment — Chat Popup, Full-Screen Assistant, and Compact Search Bar.
- - Added live previews and customizable themes in the Activation tab for all UI modes.

~ **Fixes:**
- Resolved UI conflicts between activation modes.
- Improved responsiveness and animations across devices.
- Fixed layout and scaling issues in the Full-Screen Assistant view.

## 3.1

<sup>Sept 16, 2025</sup>

~ **New:**
- Introduced Hybrid Crawler module with:
    Dynamic Site Capture for fully rendering JavaScript-heavy websites.
    Smart Performance Mode for memory-optimized processing of large HTML payloads.

~ **Fixes:**
- Added proper spacing for node plus button and considered moving button outside node container.
- Standardized size, color, and styling for “Add Revision” and “Cancel” buttons.
...
~ **New:**
- “Stop Response” button added to cancel ongoing AI responses. Input box now disables while AI is generating to prevent multiple prompts.

~ **Fixes:**
- Chat now scrolls to latest message after emoji selection.
- Fixed emoji picker behavior to close when clicking outside or pressing Enter after selection.
- Standardized size, color, and styling for “Add Revision” and “Cancel” buttons.
...
~ **New:**
- Clear All Nodes button in Sequence Studio to reset all nodes with a single click.

~ **Improvements:**
- Enhanced Change Password dialog with bigger title font, left-aligned password condition text, and blue tick marks for satisfied conditions.

~ **Fixes:**
- Adjusted spacing between labels and input fields for a cleaner layout.
- Fixed capitalization and typography in BrainVault settings; added spacing under “AI Model” textbox.
- Updated or removed broken Help icon link.
...
~ **Improvements:**
- Improved Node dialog UX — node settings modal now opens on top of existing modals for better visibility.

~ **Fixes:**
- Added missing padding to dialog boxes for improved readability.
- Added padding between transcripts and Close button in transcript modal.
- Made “View Plans” button fully responsive for smaller screens.
- Increased text size and improved color contrast for better accessibility.
...
~ **Improvements:**
- Replaced single-line chat input with a growing textarea for a smoother typing experience.

~ **Fixes:**
- Fixed chat auto-scroll behavior so it scrolls to the latest message automatically unless manually overridden.
- Prevented chat input text from overlapping attachment/send icons by adding right padding.


## 3.0.1

<sup>July 20, 2025</sup>
...
- Pinecone/Redis latency issue fixed, faster responses

## 3.0.0

<sup>July 15, 2025</sup>

~ **New:**

- New subscription pricing for more flexibility.
- Search functionality added for messages within transcripts.
- Support for multiple third-party integrations: Slack, Discord, WhatsApp, Wix, Webflow, Framer, Squarespace, and Bubble.

~ **Fixes:**

- Resolved issues with lazy onboarding.
...
- Added 12 new templates to help users easily build their own bots.
- Introduced pre-data initialization, now users can pass customer data to the bot when a chat is initiated, enabling personalized greetings using the customer’s name and other details.
- Added undo/redo functionality for improved editing experience.
- Implemented save history for system prompts.
- Added support for XLSX, CSV, and DOCX files in the Brain Vault, along with a knowledge preview feature.
- Bots now auto-set the user-prompt input for LLM nodes from the nodes they are connected to.
- Released a new version of our documentation.

~ **Improvements:**

- Prorated plans to monthly billing for subscription downgrades for better billing accuracy.
- Updated UI for the onboarding flow to enhance user experience.
- Added a confirmation modal for all delete actions to prevent accidental deletions.

~ **Fixes:**

- Fixed node menu length issue.
- Resolved promo code validation problem.
- Fixed a bug in the Condition Node.
